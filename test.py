...
def test_register(username, password):
    ...
    ), f"Ошибка регистрации {username}: {response.json()}"
...
def test_purchase(username, access_token):
    ...
    assert (
        response.status_code == 200
    ), f"Ошибка покупки для {username}: {response.json()}"
    json_response = response.json()

    assert (
        json_response.get("description") == "Успешный ответ."
    ), f"Ошибка API для {username}"
...
def test_transfer(sender_name, sender_token, recipient_name, amount):
    ...
    assert (
        response.status_code == 200
    ), f"Ошибка перевода от {sender_name} к {recipient_name}: {response.json()}"
    json_response = response.json()

    assert (
        json_response.get("description") == "Успешный ответ!"
    ), f"Ошибка API при переводе от {sender_name} к {recipient_name}"

    print(
        f"✅ {sender_name} успешно перевел {amount} монеток пользователю {recipient_name}!"
    )
